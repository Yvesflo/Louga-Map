...
const uncoveredLayer = L.layerGroup(); // NOT added by default — user activates
...
.then(([euclidianMatrix, rnMatrix, lcpMatrix, villagesAll, boundary, centres]) => {
  ...
  // ── 2. BUFFER VILLAGES ─────────────────────────────────────────────────
  // Build an exact lookup of all health-centre coordinates so we can
  // reliably identify the village point in each MultiPoint feature.
  ...
  // TECHNIQUES
  // Note: UI for technique switching is not implemented yet;
  // currently we render Euclidian distances by default.
  const activeTechnique = 'euclidian';
  const techniqueMatrices = {
    euclidian: euclidianMatrix,
    rn: rnMatrix,
    lcp: lcpMatrix,
  };

  const distMatrix = techniqueMatrices[activeTechnique];
  ...
  distMatrix.features.forEach((f, idx) => {
    ...
    try {
      ...
      if (!village || !distM) return;  // centre can be null, that's ok

      const distKm = distM / 1000;
      bufferedNames.add(village);

      // MultiPoint: one coord is the health centre, the other is the village.
      // Pick the point whose rounded key is NOT a known centre coordinate.
      ...
      if (centreCoordKeys.has(key0)) {
        // c0 is the centre → c1 is the village
        lat = c1[1]; lng = c1[0];
      } else if (centreCoordKeys.has(key1)) {
        // c1 is the centre → c0 is the village
        lat = c0[1]; lng = c0[0];
      } else {
        // Neither matches exactly (possible floating-point offset) —
        // fall back to whichever point is further from the other
        // (the village is generally further from centre than centre is from itself)
        lat = c0[1]; lng = c0[0];
      }
      if (!lat || !lng) return;
      ...
      allBuffer.push({ village, centre, distKm, lat, lng, band, props });
    } catch(e) { console.warn(`Buffer feature ${idx}:`, e.message); }
  });
  ...
  villagesAll.features.forEach((f, idx) => {
    ...
    } catch(e) { console.warn(`Uncovered feature ${idx}:`, e.message); }
  });
  ...
  centres.features.forEach(f => {
    try {
      ...
      L.marker([lat, lng], { icon: pulseIcon })
        ...
        .addTo(centresLayer);
    } catch(e) { console.warn('Centre error:', e.message); }
  });

  document.getElementById('s-centres').textContent = centres.features.length;

  // ── 5. BUILD SEARCH INDEX ──────────────────────────────────────────────
  // Index buffer villages with all searchable fields
  ...
  // Index uncovered villages with admin fields
...
function renderBufferVillages() {
  ...
  allBuffer.forEach(v => {
    ...
    marker.on('mouseout', function () { this.setStyle({ radius: 6, weight: 0.8 }); this.closeTooltip(); });
...
function renderUncoveredVillages() {
  ...
  allUncovered.forEach(v => {
    ...
    marker.on('mouseout', function () { this.setStyle({ radius: 1.5 }); this.closeTooltip(); });
    marker.on('click', function () {
      marker.bindPopup(`
...
function buildChart() {
  chart = new Chart(document.getElementById('distChart'), {
    ...
    data: {
      ...
      datasets: [{ data: BANDS.map(b => allBuffer.filter(v => v.distKm >= b.min && v.distKm < b.max).length),
        backgroundColor: BANDS.map(b => b.color), borderWidth: 1, borderColor: '#ccc', borderRadius: 4 }]
    },
    options: {
...
// ── EXPORT: PNG (Screen Capture API) ─────────────────────────────────────────
// Uses the browser's native getDisplayMedia API — captures exactly what is
// rendered on screen without any DOM-transform issues.
async function exportPNG() {
  ...
  try {
    const stream = await navigator.mediaDevices.getDisplayMedia({
      ...
      preferCurrentTab: true,   // Chrome 107+: pre-selects the current tab
    });
...
// The browser renders the map natively for print — perfectly aligned,
// no DOM-parsing hacks. @media print CSS hides the sidebar and injects
// a branded header. User chooses "Save as PDF" in the print dialog.
function exportPDF() {
...
function exportCSV() {
  ...
  const link = document.createElement('a');
  link.download = 'LougaMap_villages.csv';
  link.href = url;
  link.click();
